...
knitr::opts_chunk$set(echo = TRUEE)
```
...
zeta1_sub_odd <- zeta1_sub[-c(8,19,21,22,25,29,31,35,36,39,47:50,54,57,58)]
zeta0_sub_odd <- zeta0_sub[-c(8,19,21,22,25,29,31,35,36,39,47:50,54,57,58)]
...
```
    ...
    Response: The odd points are from schoolids 1, 4, 5, 9, 10, 12, 14, 16, 17, 19, 20, 22, 23, 24, 26, 28, 33, 38, 42, 43, 45, 46, 47, 52, 57, 60, 61, 69, 73, 78, 79, 80, 84, 86, 89, 90, 96, 98, 100, 102, 103, and 106. It appears that they all have MINORITY=0 in common.
...
```{r}
# Insert code to fit model and print summary
fit.M00 <- lmer(math ~ year + (1 | schoolid), data = class_pp)
...
$$
and we assume ${\zeta_{0k}} \sim N(0,\sigma_{{\zeta_0}}^2)$ and ${\varepsilon_{tijk}} \sim N(0,\sigma _\varepsilon ^2)$, independently.
    
### c. Add random intercepts for child
```{r}
# Insert code to fit new model and print summary output
fit.M0 <- lmer(math ~ year + (1 | schoolid/childid), data = class_pp)
summary(fit.M0)
```
...
$\sigma^2_{\zeta_0}:$ The original variance estimate is 348.7. The new variance estimate is 307.5.
$\sigma^2_{\varepsilon}:$ The original variance estimate is 1268.4. The new variance estimate is 599.1.
...
(348.7-307.5)/348.7
```
    Response: The proportion of the between-school variance in the first model that is ‘explained’ by the addition of a student random effect is 0.118.
...
```
    ...
    Response: Yes, the total variation stays about the same. The total variation of the first model is 1617.1. The total variation of the second model is 1608.6.
...
fit.M1 <- lmer(math ~ year + (0 + year | schoolid) + (1 | schoolid/childid), data = class_pp)
summary(fit.M1)
